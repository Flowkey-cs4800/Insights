...
export const getMetricTypes = () => 
  callApi<MetricType[]>('/api/metric-types');

export const createMetricType = (name: string, kind: string, unit?: string) =>
  callApi<MetricType>('/api/metric-types', 'POST', { name, kind, unit });
...
export const compareMetrics = (metricTypeIdX: string, metricTypeIdY: string) =>
  callApi<CompareResult>(`/api/metrics/compare?metricTypeIdX=${metricTypeIdX}&metricTypeIdY=${metricTypeIdY}`);